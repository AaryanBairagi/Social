...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    try{
      ...
      if(!data) throw new Error("Error getting the pending requests");

      const receivedRequests = data?.receivedRequests || data?.requests || data || [];

      SetHasPendingRequests(Array.isArray(receivedRequests) && receivedRequests.length>0); 
    }catch(err){
    ...
    { title: "Connections", 
      icon: (
        <div className='relative'>
          <Users />
          {hasPendingRequests && (
            <span className='absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-red-400 ring-2 ring-white' />
          )}
        </div>
      ), 
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/10 backdrop-blur-md border-b border-gray-500/40 
      ...
      <div className="flex items-center gap-2 min-w-max ml-2">
        ...
        <span className='font-bold text-gray-800 text-xl'><Link href="/">Social</Link></span>
      </div>
